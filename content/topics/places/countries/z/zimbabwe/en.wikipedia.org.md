---
title: "Zimbabwe (Wikipedia)"
borders:
  - Mozambique (Wikipedia)
  - Zambia (Wikipedia)
  - incomplete list
country of: Africa (Wikipedia)
excerpt: >-
  **Zimbabwe** (/zɪmˈbɑːbweɪ, -wi/; Shona pronunciation: [zi.ᵐba.ɓwe]), officially the **Republic of Zimbabwe**, is a landlocked country in Southern Africa, between the Zambezi and Limpopo Rivers, bordered by South Africa to the south, Botswana to the southwest, [Zambia](/en.wikipedia.org/wiki/Zambia/) to the north, and [Mozambique](/en.wikipedia.org/wiki/Mozambique/) to the east. The capital and largest city is Harare, and the second largest is Bulawayo.
flag: Flag of Zimbabwe (Wikimedia Commons)
retrieved: 2024-07-03
type: website
url: /en.wikipedia.org/wiki/Zimbabwe/
website: "https://en.wikipedia.org/wiki/Zimbabwe"
tags:
  - Zimbabwe
  - Landlocked (Wikipedia)
  - Country (Wikipedia)
  - Wikipedia
---