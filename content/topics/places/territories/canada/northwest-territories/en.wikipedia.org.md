---
title: "Northwest Territories (en.wikipedia.org)"
borders:
  - Alberta (en.wikipedia.org)
  - British Columbia (en.wikipedia.org)
  - Manitoba (en.wikipedia.org)
  - Nunavut (en.wikipedia.org)
  - Saskatchewan (en.wikipedia.org)
  - Yukon (en.wikipedia.org)
excerpt: >-
  The **Northwest Territories** (abbreviated **NT** or **NWT**; French: *Territoires du Nord-Ouest*; formerly **North-West Territories**) is a federal territory of [Canada](/en.wikipedia.org/wiki/Canada/). At a land area of approximately 1,144,000 km2 (442,000 sq mi) and a 2016 census population of 41,790, it is the second-largest and the most populous of the three territories in Northern Canada. Its estimated population as of 2023 is 45,668. Yellowknife is the capital, most populous community, and only city in the territory; its population was 19,569 as of the 2016 census. It became the territorial capital in 1967, following recommendations by the Carrothers Commission.
related: Flag_of_the_Northwest_Territories.svg (commons.wikimedia.org)
retrieved: 2024-04-13
territory of: Canada (en.wikipedia.org)
type: website
url: /en.wikipedia.org/wiki/Northwest_Territories/
website: "https://en.wikipedia.org/wiki/Northwest_Territories"
tags:
  - Northwest Territories
  - Wikipedia
---